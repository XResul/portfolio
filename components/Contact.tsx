// components/Contact.tsx
// İletişim bölümü - Form ve sosyal medya linkleri

import ScrollAnimation from "./ScrollAnimation";

export default function Contact() {
  return (
    <section id="contact" className="py-20 bg-slate-800">
      <div className="max-w-6xl mx-auto px-4">
        {/* Bölüm Başlığı */}
        <ScrollAnimation>
          <h2 className="text-4xl font-bold text-center mb-4 text-cyan-400">
            İletişim
          </h2>
          <p className="text-center text-gray-400 mb-12 max-w-2xl mx-auto">
            Benimle iletişime geçmek için formu doldurabilir veya sosyal medya
            hesaplarımdan ulaşabilirsiniz.
          </p>
        </ScrollAnimation>

        <div className="grid md:grid-cols-2 gap-12">
          {/* Sol Taraf - İletişim Formu */}
          <ScrollAnimation delay={100}>
            <div className="bg-slate-900 p-8 rounded-2xl">
              <h3 className="text-xl font-semibold text-white mb-6">
                Mesaj Gönder
              </h3>

              <form className="space-y-4">
                {/* İsim */}
                <div>
                  <label className="block text-gray-400 mb-2 text-sm">
                    İsim
                  </label>
                  <input
                    type="text"
                    placeholder="Adınız Soyadınız"
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 transition-colors"
                    // focus:outline-none: Varsayılan mavi çerçeveyi kaldır
                    // focus:border-cyan-500: Focus olunca cyan çerçeve
                  />
                </div>

                {/* Email */}
                <div>
                  <label className="block text-gray-400 mb-2 text-sm">
                    Email
                  </label>
                  <input
                    type="email"
                    placeholder="ornek@email.com"
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 transition-colors"
                  />
                </div>

                {/* Mesaj */}
                <div>
                  <label className="block text-gray-400 mb-2 text-sm">
                    Mesaj
                  </label>
                  <textarea
                    rows={5}
                    placeholder="Mesajınızı yazın..."
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 transition-colors resize-none"
                    // resize-none: Kullanıcı boyutunu değiştiremesin
                  />
                </div>

                {/* Gönder Butonu */}
                <button
                  type="submit"
                  className="w-full py-3 bg-cyan-600 text-white rounded-lg font-medium hover:bg-cyan-500 transition-colors"
                >
                  Gönder
                </button>
              </form>
            </div>
          </ScrollAnimation>

          {/* Sağ Taraf - İletişim Bilgileri */}
          <ScrollAnimation delay={200}>
            <div className="space-y-8">
              {/* Bilgi Kartları */}
              <div className="space-y-4">
                {/* Email */}
                <div className="flex items-center gap-4 p-4 bg-slate-900 rounded-xl">
                  <div className="w-12 h-12 bg-cyan-600/20 rounded-lg flex items-center justify-center">
                    <svg
                      className="w-6 h-6 text-cyan-400"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                      />
                    </svg>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Email</p>
                    <p className="text-white">resulbinici222@gmail.com</p>
                  </div>
                </div>

                {/* Konum */}
                <div className="flex items-center gap-4 p-4 bg-slate-900 rounded-xl">
                  <div className="w-12 h-12 bg-cyan-600/20 rounded-lg flex items-center justify-center">
                    <svg
                      className="w-6 h-6 text-cyan-400"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                      />
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                      />
                    </svg>
                  </div>
                  <div>
                    <p className="text-gray-400 text-sm">Konum</p>
                    <p className="text-white">Türkiye</p>
                  </div>
                </div>
              </div>

              {/* Sosyal Medya Linkleri */}
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">
                  Sosyal Medya
                </h3>
                <div className="flex gap-4">
                  {/* GitHub */}
                  <a
                    href="https://github.com/XResul"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center text-gray-400 hover:text-cyan-400 hover:bg-slate-700 transition-all"
                  >
                    <svg
                      className="w-6 h-6"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                    </svg>
                  </a>

                  {/* LinkedIn */}
                  <a
                    href="https://linkedin.com/in/kullaniciadi"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center text-gray-400 hover:text-cyan-400 hover:bg-slate-700 transition-all"
                  >
                    <svg
                      className="w-6 h-6"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                    </svg>
                  </a>

                  {/* Twitter/X */}
                  <a
                    href="https://twitter.com/kullaniciadi"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center text-gray-400 hover:text-cyan-400 hover:bg-slate-700 transition-all"
                  >
                    <svg
                      className="w-6 h-6"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </ScrollAnimation>
        </div>
      </div>
    </section>
  );
}
